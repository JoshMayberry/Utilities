                ...
                for x in range(8):
                        for y in range(8):
                                if board[x][y] != newBoard[x][y]: #Looking for change in board layout
                                        print 'I found a change. ( board:',board[x][y],' & newBoard:',newBoard[x][y], ') At', x, '&', y
                                        return (x,y)
                #No change was found
                return []
        ...
        def GetPlayerInput(self,board,currentColor):
                ...
                while True:
                        ...
                        while squareClicked == []: #Wait for user to pick move
                                input_state = False#GPIO.input(turnBtn_pin)
                                if input_state == False:
                                        #print 'button pressed'
                                        squareClicked = self.CheckBoard(board)
                                        time.sleep(0.2)
                                        fromTuple = squareClicked
                                        toTuple = squareClicked
                        print 'I am out of the loop'
                        fromTuple, toTuple = self.GetToAndFrom()
                        break
...
##
                print 'I am finished'
                print "I moved from", fromTuple, "to", toTuple
                return (fromTuple,toTuple)
